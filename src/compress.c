...
int compress_get_bits(compress *c, int length)
{
        int i, end_pos_in_bits, start_pos_in_bytes,
            end_pos_in_bytes, value, mask_sample_bits, unused_bits, original;
...
void compress_load_character_length_huffman(compress *c)
{
        ...
        if (count == 0) {
                ...
                for (i = 0; i < count; i++) {
                        ...
                        c->character_length_huffman.lengths[i] =
                            compress_read_variable_length(c);
                }
        }
...
int hus_decompress(char *data, int length, int8_t *output, int *output_length)
{
        ...
        while (c->bits_total > c->bit_position && i < *output_length) {
                ...
                if (character < 0x100) {
                        ...
                        for (j = c->bit_position; j < c->bit_position + length;
                             j++) {
                                output[i] = output[j];
                                i++;
                        }
