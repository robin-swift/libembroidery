...
void jefSetHoopFromId(EmbPattern *pattern, int hoopCode)
{
    if (!pattern) {
        printf
            ("ERROR: format-jef.c jefSetHoopFromId(), pattern argument is null\n");
        return;
    }
    switch (hoopCode) {
    case HOOP_126X110:
...
int8_t readJef(EmbPattern *pattern, FILE *file)
{
    ...
    for (i = 0; i < numberOfColors; i++) {
        ...
        embp_addThread(pattern, jef_colors[thread_num % 79]);
    }
    ...
    while (stitchCount < numberOfStitchs + 100) {
        ...
        if (b[0] == 0x80) {
            if (b[1] & 1) {
                ...
                embp_addStitchRel(pattern, 0.0, 0.0, END, 1);
                break;
            }
        }
        dx = jefDecode(b[0]);
...
int8_t writeJef(EmbPattern *pattern, FILE *file)
{
    ...
    embp_correctForMaxStitchLength(pattern, 12.7, 12.7);
    ...
    data =
        pattern->stitch_list->count + EMB_MAX(0, (6 - colorlistSize) * 2) + 1;
    ...
    for (i = 0; i < pattern->thread_list->count; i++) {
        int j = emb_find_nearest_thread(pattern->thread_list->thread[i].color,
                                        (EmbThread *) jef_colors,
                                        79);
