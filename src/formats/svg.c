...
EmbColor svgColorToEmbColor(char *colorString)
{
    ...
    if (length == 7 && colorStr[0] == '#') {    /* Six digit hex — #rrggbb */
        ...
        c.r = (uint8_t) emb_round(255.0 / 100.0 * strtod(colorStr, &pEnd));
        c.g = (uint8_t) emb_round(255.0 / 100.0 * strtod(pEnd, &pEnd));
        c.b = (uint8_t) emb_round(255.0 / 100.0 * strtod(pEnd, &pEnd));
                ...
                if (trip >= 0) {
                    ...
                    printf
                        ("*prior:%s (%f, %f,  %f, %f,     %f,%f,  %f) \n",
                         ...
                         pathData[4], pathData[5], pathData[6]);
                }
    ...
    if (svgExpect == SVG_EXPECT_ELEMENT) {
        ...
        if (!advance) {
            if (stringInArray(buff, (const int8_t **)
                              svg_attribute_table[current_element_id])) {
    ...
    for (i = 0; i < pattern->geometry->count; i++) {
        ...
        case EMB_POLYGON:{
                ...
                fprintf(file,
                        ...
                        color.r, color.g, color.b,
                        emb_optOut(pointList->geometry[0].object.point.position.
                                   x, tmpX),
                        emb_optOut(pointList->geometry[0].object.point.position.
                                   y, tmpY));
                for (j = 1; j < pointList->count; j++) {
                    fprintf(file, " %s,%s",
                            emb_optOut(pointList->geometry
                                       [j].object.point.position.x, tmpX),
                            emb_optOut(pointList->geometry
                                       [j].object.point.position.y, tmpY));
                }
                fprintf(file, "\"/>");
                break;
            }
