...
void parse_path(EmbPattern *p)
{
        ...
        if (!pathbuff) {
                printf
                    ("ERROR: svgAddToPattern(), cannot allocate memory for pathbuff\n");
                return;
        }
        ...
        for (i = 0; i < last; i++) {
                        ...
                        while (pendingTask > 0) {
                                ...
                                if (trip >= 0) {
                                        ...
                                        if (!pointList && !flagList) {
                                                pointList =
                                                    emb_array_create(EMB_POINT);
                                                flagList =
                                                    emb_array_create(EMB_FLAG);
                                        }
                                        test.position = position;
                                        emb_array_addPoint(pointList, test);
                                        emb_array_add_flag(flagList,
                                                           svgPathCmdToEmbPathFlag
                                                           (cmd));
                                        l_point = position;
                                        ...
                                        printf
                                            ("*prior:%s (%f, %f,  %f, %f,     %f,%f,  %f) \n",
                                             pathbuff, pathData[0], pathData[1],
                                             pathData[2], pathData[3],
                                             pathData[4], pathData[5],
                                             pathData[6]);
                                }
                                ...
                                if (trip == -1 && reset == -1) {
                                        ...
                                        if (c == 'M' || c == 'm' || c == 'L'
                                            || c == 'l') {
                                                ...
                                                printf
                                                    ("ERROR: svgAddToPattern(), %s is not a valid svg path command, skipping...\n",
                                                     pathbuff);
                                                trip = -1;
                                                reset = -1;
                                        }
                ...
                if (pos >= size - 1) {
                        ...
                        if (!pathbuff) {
                                printf
                                    ("ERROR: svgAddToPattern(), cannot re-allocate memory for pathbuff\n");
                                return;
                        }
                }
        }
...
EmbArray *parse_pointlist(EmbPattern *p)
{
        ...
        if (!polybuff) {
                printf
                    ("ERROR: svgAddToPattern(), cannot allocate memory for polybuff\n");
                return pointList;
        }
        for (i = 0; i < last; i++) {
                int8_t c = pointStr[i];
                ...
                if (pos >= size - 1) {
                        ...
                        if (!polybuff) {
                                printf
                                    ("ERROR: svgAddToPattern(), cannot re-allocate memory for polybuff\n");
                                return NULL;
                        }
                }
        }
...
void parse_polygon(EmbPattern *p)
{
        ...
        /*
           ...
           embp_addPolygonObjectAbs(p, &polygonObj);
         */
}
...
void parse_polyline(EmbPattern *p)
{
        ...
        /* BROKEN
           ...
           embp_addPolylineObjectAbs(p, polylineObj);
         */
}
...
void svgProcess(int c, const int8_t *buff)
{
        if (svgExpect == SVG_EXPECT_ELEMENT) {
                ...
                if (!advance) {
                        ...
                        printf
                            ("ERROR %s not found in svg_attribute_table[%d].\n",
                ...
                if ((buff[0] == '"' || buff[0] == '\'')
                    && (buff[last] == '/' || buff[last] == '"'
                        || buff[last] == '\'') && !svgMultiValue) {
                        svgExpect = SVG_EXPECT_ATTRIBUTE;
                        strcpy(attributeList[n_attributes].name,
                               currentAttribute);
                        ...
                        if (buff[last] == '/' || buff[last] == '"'
                            || buff[last] == '\'') {
                                ...
                                strcpy(attributeList[n_attributes].name,
                                       currentAttribute);
                                strcpy(attributeList[n_attributes].value,
                                       currentValue);
                                n_attributes++;
                        }
                }
...
int8_t readSvg(EmbPattern *pattern, FILE *file)
{
        ...
        while (fread(&c, 1, 1, file)) {
                ...
                if (pos >= size - 1) {
                        ...
                        if (!buff) {
                                printf
                                    ("ERROR: readSvg(), cannot re-allocate memory for buff\n");
                                return 0;
                        }
                }
        }
        ...
        if (emb_verbose > 1) {
                ...
                if (pattern->ellipses) {
                        for (i = 0; i < pattern->ellipses->count; i++) {
                                ...
                                printf("ellipse %f %f %f %f\n", e.center.x,
                                       e.center.y, e.radius.x, e.radius.y);
                        }
                }
                if (pattern->geometry) {
                        for (i = 0; i < pattern->geometry->count; i++) {
                                EmbLine li =
                                    pattern->geometry->geometry[i].object.line;
                                printf("line %f %f %f %f\n", li.start.x,
                                       li.start.y, li.end.x, li.end.y);
                        }
                }
                if (pattern->points) {
                        for (i = 0; i < pattern->points->count; i++) {
                                EmbVector po =
                                    pattern->points->point[i].position;
                                printf("point %f %f\n", po.x, po.y);
                        }
                }
                if (pattern->polygons) {
                        for (i = 0; i < pattern->polygons->count; i++) {
                                EmbArray *verts =
                                    pattern->polygons->polygon[i].pointList;
                                printf("polygon %d\n", verts->count);
                        }
                }
                if (pattern->polylines) {
                        for (i = 0; i < pattern->polylines->count; i++) {
                                EmbArray *verts =
                                    pattern->polylines->polyline[i].pointList;
                                printf("polyline %d\n", verts->count);
                        }
                }
                if (pattern->rects) {
...
int8_t writeSvg(EmbPattern *pattern, FILE *file)
{
        ...
        for (i = 0; i < pattern->geometry->count; i++) {
                ...
                case EMB_POLYGON:{
                                ...
                                fprintf(file,
                                        ...
                                        color.r, color.g, color.b,
                                        emb_optOut(pointList->
                                                   geometry[0].object.point.
                                                   position.x, tmpX),
                                        emb_optOut(pointList->
                                                   geometry[0].object.point.
                                                   position.y, tmpY));
                ...
                case EMB_RECT:{
                                ...
                                fprintf(file,
                                        ...
                                        color.r, color.g, color.b, rect.x,
        ...
        for (i = 0; i < pattern->stitch_list->count; i++) {
                ...
                if (st.flags == NORMAL && !isNormal) {
                        ...
                        fprintf(file,
                                "\n<polyline stroke-linejoin=\"round\" stroke-linecap=\"round\" stroke-width=\"0.2\" stroke=\"#%02x%02x%02x\" fill=\"none\" points=\"%s,%s",
                                color.r, color.g, color.b, emb_optOut(st.x,
                                                                      tmpX),
                                emb_optOut(st.y, tmpY));
                } else if (st.flags == NORMAL && isNormal) {
                        fprintf(file, " %s,%s", emb_optOut(st.x, tmpX),
                                emb_optOut(st.y, tmpY));
                } else if (st.flags != NORMAL && isNormal) {
                        isNormal = 0;
                        fprintf(file, "\"/>");
...
int8_t writeT01(EmbPattern *pattern, FILE *file)
{
        ...
        boundingRect = embp_bounds(pattern);
        if (emb_verbose > 1) {
                printf("bounding rectangle with top %f not used ",
                       boundingRect.x);
