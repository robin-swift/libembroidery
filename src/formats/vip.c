...
{
    uint8_t *decompressedData = (uint8_t *) malloc(decompressedContentLength);
    if (!decompressedData) {
        printf
...
int8_t readVip(EmbPattern *pattern, FILE *file)
{
    ...
    uint8_t *xData = 0, *xDecompressed = 0, *yData = 0, *yDecompressed = 0;
    VipHeader header;
    ...
    for (i = 0; i < header.numberOfColors * 4; ++i) {
        ...
        uint8_t tmpByte = (uint8_t) (inputByte ^ vipDecodingTable[i]);
    ...
    attributeData = (uint8_t *) malloc(header.xOffset - header.attributeOffset);
    if (!attributeData) {
        printf
...
uint8_t *vipCompressData(uint8_t *input, int decompressedInputSize,
                         int *compressedSize)
{
    uint8_t *compressedData = (uint8_t *) malloc(decompressedInputSize * 2);
    if (!compressedData) {
        printf
...
int8_t writeVip(EmbPattern *pattern, FILE *file)
{
    ...
    emb_write_i16(file,
                  (int16_t) -
                  emb_round((boundingRect.y + boundingRect.h) * 10.0 - 1.0));
    ...
    if (xValues && yValues && attributeValues) {
        ...
        attributeCompressed =
            vipCompressData(attributeValues, stitchCount, &attributeSize);
        ...
        emb_write_u32(file,
                      (uint32_t) (0x38 + (minColors << 3) + attributeSize));
        emb_write_u32(file,
        ...
        for (i = 0; i < minColors << 2; ++i) {
            uint8_t tmpByte =
                (uint8_t) (decodedColors[i] ^ vipDecodingTable[i]);
