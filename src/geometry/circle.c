/**
 * @file circle.c
 * @brief Functions related to the EmbCircle struct.
 */
...
/*
 */
EmbCircle emb_circle(EmbReal x, EmbReal y, EmbReal radius)
{
    EmbCircle circle;
    circle.center.x = x;
    circle.center.y = y;
                             ...
                             EmbVector *p0, EmbVector *p1)
{
    ...
    a = ((c0.radius * c0.radius) - (c1.radius * c1.radius) +
         (d * d)) / (2.0 * d);
 ...
 */
int
getCircleTangentPoints(EmbCircle c, EmbVector point, EmbVector *t0,
                       EmbVector *t1)
{
    EmbCircle p;
    EmbReal hyp;
