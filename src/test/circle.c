                   ...
                   EmbReal tolerence)
{
        ...
        double error =
            emb_vector_distance(p0, s0) + emb_vector_distance(p1, s1);
        if (error > tolerence) {
...
EmbError circle_tests(void)
{
        ...
        EmbCircle c1 = emb_circle(0.0f, 0.0f, 3.0f);
...
{
        EmbCircle circle = emb_circle(0.0, 0.0, 10.0);
        EmbReal error = fabs(emb_circle_area(circle) - 314.159);
        if (error > 0.01) {
                return 1;
        }
        if (circle_tests()) {
