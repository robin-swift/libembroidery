...
int test_convert(int test_file, int from, int to)
{
        ...
        EmbString fname_to;
        sprintf(fname_from, "test%02d%s", test_file,
                formatTable[from].extension);
        sprintf(fname_to, "test%02d_out%s", test_file,
                formatTable[to].extension);
        printf("Converting test case %d: %s to %s.\n", test_file, fname_from,
               fname_to);
        ...
        case 0:{
                        ...
                        for (i = 0; i < 200; i++) {
                                ...
                                st.flags = NORMAL;
                                st.color = 0;
                                embp_addStitchAbs(p, st.x, st.y, st.flags,
                                                  st.color);
                        }
                        break;
                }
